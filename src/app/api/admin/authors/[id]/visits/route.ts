...
import {
  getAuthorViews,
  getAuthorViewStats,
  getAuthorViewsByDate,
} from '@/lib/lms/repositories/author-view.repository'
...
/**
 * GET /api/admin/authors/[id]/visits
 * Get visit analytics data for an author
 */
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (chart) {
      ...
      const chartData = await getAuthorViewsByDate(authorId, start, end)
    ...
    const visits = await getAuthorViews(authorId, {
      page,
      limit,
    ...
    console.error('Error fetching author visits:', error)
    return NextResponse.json(
