...
/**
 * POST /api/books/[id]/regenerate-audiobook
 * Regenerate audiobook for a book using the PDF Processor service
 */
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    const { id } = await params;

    console.log('[Regenerate Audiobook API] Starting regeneration for book:', id);
    ...
    console.log('[Regenerate Audiobook API] Checking if job exists:', jobCheckUrl);

    const jobCheckResponse = await fetch(jobCheckUrl, { headers });
    let result;

    if (jobCheckResponse.ok) {
      // Job exists, trigger selective audiobook regeneration
      console.log('[Regenerate Audiobook API] Job exists, triggering audiobook regeneration');
      const regenerateUrl = `${pdfProcessorUrl}/api/job/${id}/regenerate-audiobook`;
      ...
      if (!regenerateResponse.ok) {
        ...
        console.error('[Regenerate Audiobook API] PDF processor regenerate error:', errorText);
        return NextResponse.json(
      ...
      console.log('[Regenerate Audiobook API] Job does not exist, creating new job');
      ...
      if (!processResponse.ok) {
        ...
        console.error('[Regenerate Audiobook API] PDF processor process error:', errorText);
        return NextResponse.json(
    ...
    console.log('[Regenerate Audiobook API] PDF processor response:', result);

    return NextResponse.json({
      message: 'Audiobook regeneration triggered successfully',
      status: 'pending',
      pdfProcessorResponse: result,
    });
  } catch (error: any) {
    console.error('[Regenerate Audiobook API] Error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to regenerate audiobook' },
      { status: 500 }
    );
  }
}
