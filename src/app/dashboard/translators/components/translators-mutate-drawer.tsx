...
import { createTranslator, updateTranslator } from '../actions'
import {
  checkTranslatorNameAvailability,
} from '../actions'
...
interface Props {
  ...
  currentRow?: Translator
  onSuccess?: () => void
}

const formSchema = z.object({
  name: z.string().min(1, 'Translator name is required.'),
  description: z.string().optional(),
  image: z.union([z.string(), z.any()]).optional(),
})

type TranslatorForm = z.infer<typeof formSchema>

export function TranslatorsMutateDrawer({ open, onOpenChange, currentRow, onSuccess }: Props) {
  ...
  const form = useForm<TranslatorForm>({
  ...
  useEffect(() => {
    const checkName = async () => {
      ...
      try {
        const result = await checkTranslatorNameAvailability(nameValue, currentRow?.id)
        if (result.isAvailable) {
          ...
          form.setError('name', {
            type: 'manual',
            message: result.error || 'Translator name is not available',
          })
        }
      } catch (error) {
        console.error('Translator name check failed:', error)
      } finally {
  ...
  const onSubmit = async (data: TranslatorForm) => {
    setLoading(true)
    try {
      ...
      if (isUpdate && currentRow) {
        await updateTranslator(currentRow.id, formData)
        toast({
          title: 'Translator updated successfully',
        })
      } else {
        await createTranslator(formData)
        toast({
          title: 'Translator created successfully',
        })
      }
      onSuccess?.()
      onOpenChange(false)
      ...
      toast({
        title: 'Error',
        description: error.message || `Failed to ${isUpdate ? 'update' : 'create'} translator`,
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
    ...
    >
      <SheetContent className='flex flex-col'>
        <SheetHeader className='text-left'>
          <SheetTitle>{isUpdate ? 'Update' : 'Create'} Translator</SheetTitle>
          <SheetDescription>
            {isUpdate
              ? 'Update the translator by providing necessary info.'
              : 'Add a new translator by providing necessary info.'}
        ...
        <Form {...form}>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel>Translator Name <span className="text-destructive">*</span></FormLabel>
                  <FormControl>
                ...
                <FormItem className='space-y-1'>
                  ...
                  <FormControl>
                    <MDXEditor
                      ...
                      placeholder='Enter translator description (e.g., biography, works, achievements) in markdown format...'
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Translator Image</FormLabel>
                  <FormControl>
                    <ImageUpload
                      value={field.value}
                      onChange={field.onChange}
