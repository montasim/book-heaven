...
function TranslatorDetailSkeleton() {
  return (
...
function UserLinkButton({ user, className = '', router }: { user: any; className?: string; router: ReturnType<typeof useRouter> }) {
  if (!user) return null
  return (
    <button
...
export default function AdminTranslatorDetailsPage() {
  ...
  // Fetch translator details with optimized caching
  const { data: translatorData, isLoading, error } = useSWR(
    `/api/admin/translators/${translatorId}/details`,
    fetcher,
    {
  ...
  const translator = translatorData?.data

  if (isLoading) {
    return <TranslatorDetailSkeleton />
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <p className="text-red-500 mb-4">Failed to load translator details</p>
  ...
  const imageUrl = translator.directImageUrl || getProxiedImageUrl(translator.image) || translator.image || '/placeholder-translator.png'

  return (
    <div className="bg-background h-screen overflow-y-auto no-scrollbar pb-4">
      {/* Action Buttons */}
      <div className="flex lg:justify-end justify-between gap-2 mb-4">
        <Button variant="outline" size="sm" onClick={() => router.push(`/translators/${translator.id}`)}>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6">
            {/* Translator Title and Description */}
            <div className="space-y-4">
              <h1 className="text-xl font-bold">{translator.name}</h1>

              {translator.description && (
                <p className="text-sm text-muted-foreground line-clamp-3">{translator.description}</p>
              )}
            </div>

            {/* Metadata - Responsive Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4 text-sm">
              <MetadataItem label="Total Spend:" value={`৳${translator.analytics?.totalSpend?.toLocaleString() || 0}`} />
              <MetadataItem label="Hard Copy:" value={translator.analytics?.booksByType?.HARD_COPY || 0} />
              <MetadataItem label="eBook:" value={translator.analytics?.booksByType?.EBOOK || 0} />
              <MetadataItem label="Audio:" value={translator.analytics?.booksByType?.AUDIO || 0} />
              <MetadataItem label="Total Books:" value={translator.analytics?.totalBooks || 0} />
              {translator.birthDate && (
                <MetadataItem label="Birth Date:" value={new Date(translator.birthDate).toLocaleDateString()} />
              )}
              {translator.nationality && (
                <MetadataItem label="Nationality:" value={translator.nationality} valueClassName="capitalize" />
              )}
              {translator.website && (
                <MetadataItem label="Website:" value={
                  <a href={translator.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all">
                    {translator.website}
                  </a>
                } />
              )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4">
              <AdminMetadataItem
                label="ID:"
                value={<span className="font-mono">{translator.id.slice(0, 8)}...</span>}
              />
              {translator.createdAt && (
                <AdminMetadataItem
                  label="Created:"
                  value={new Date(translator.createdAt).toLocaleString()}
                />
              )}
              {translator.updatedAt && (
                <AdminMetadataItem
                  label="Last Updated:"
                  value={new Date(translator.updatedAt).toLocaleString()}
                />
              )}
              {translator.entryBy && (
                <AdminMetadataItem label="Entered By:" value={
                  <div className="flex items-center gap-2 -mt-1">
                    <Avatar className="h-4 w-4">
                      <AvatarImage src={translator.entryBy.directAvatarUrl} />
                      <AvatarFallback className="text-[10px]">
                        {translator.entryBy.name?.[0] || translator.entryBy.username?.[0] || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    <UserLinkButton user={translator.entryBy} router={router} />
                  </div>
                } />
              )}
            </div>
            ...
            <DashboardSummary
              summaries={[
                {
                  title: 'Total Views',
                  value: translator.analytics?.totalViews || 0,
                  description: 'All time views',
                  icon: Eye,
                },
                {
                  title: 'Books',
                  value: translator.analytics?.totalBooks || 0,
                  description: `${translator.analytics?.totalPages || 0} total pages`,
                  icon: BookOpen,
                },
                {
                  title: 'Readers',
                  value: translator.analytics?.totalReaders || 0,
                  description: 'Total readers',
                  icon: Users,
                },
                {
                  title: 'Completed Books',
                  value: translator.analytics?.completedReaders || 0,
                  description: 'Finished reading',
                  icon: BookOpen,
                },
                {
                  title: 'Avg Completion',
                  value: translator.analytics?.totalReaders
                    ? `${Math.round((translator.analytics.completedReaders / translator.analytics.totalReaders) * 100)}%`
                    : 'N/A',
        ...
        <Tabs value={activeTab} className="space-y-4">
          <TabsList className="overflow-x-auto">
            <Link href={`/dashboard/translators/${translatorId}?tab=books`}>
              <TabsTrigger value="books">Books</TabsTrigger>
            </Link>
            <Link href={`/dashboard/translators/${translatorId}?tab=analytics`}>
              <TabsTrigger value="analytics">Analytics</TabsTrigger>
            </Link>
            <Link href={`/dashboard/translators/${translatorId}?tab=readers`}>
          ...
          <TabsContent value="books">
            <BooksByTranslatorTab translatorId={translatorId} activeTab={activeTab} />
          </TabsContent>

          {/* Analytics Tab */}
          <TabsContent value="analytics">
            <AnalyticsTab translatorId={translatorId} activeTab={activeTab} />
          </TabsContent>

          {/* Readers Tab */}
          <TabsContent value="readers">
            <ReadersTab translatorId={translatorId} activeTab={activeTab} />
          </TabsContent>
        </Tabs>
      </div>

      {/* Edit Translator Drawer */}
      {translator && (
        <TranslatorsMutateDrawer
          key={`edit-translator-${translatorId}`}
          open={isEditDrawerOpen}
          onOpenChange={setIsEditDrawerOpen}
          currentRow={translator as unknown as Translator}
          onSuccess={() => {
            mutate(`/api/admin/translators/${translatorId}/details`)
...
// Books Tab Component
function BooksByTranslatorTab({ translatorId, activeTab }: { translatorId: string; activeTab: string }) {
  ...
  const swrKey = shouldFetch
    ? `/api/admin/translators/${translatorId}/books?page=${pagination.pageIndex + 1}&limit=${pagination.pageSize}`
    : null
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Books by Translator</CardTitle>
...
function AnalyticsTab({ translatorId, activeTab }: { translatorId: string; activeTab: string }) {
  // Only fetch when this tab is active
  const shouldFetch = activeTab === 'analytics'

  const { data, isLoading: isLoadingChart } = useSWR(
    shouldFetch ? `/api/admin/translators/${translatorId}/visits?chart=true` : null,
    (url) => fetch(url).then(r => r.json()),
    {
      revalidateOnFocus: false,
      dedupingInterval: 30000,
  ...
  const { data: visitsData, isLoading: isLoadingVisits } = useSWR(
    shouldFetch ? `/api/admin/translators/${translatorId}/visits?limit=10` : null,
    (url) => fetch(url).then(r => r.json()),
    {
      revalidateOnFocus: false,
      dedupingInterval: 30000,
...
function ReadersTab({ translatorId, activeTab }: { translatorId: string; activeTab: string }) {
  // Only fetch when this tab is active
  const shouldFetch = activeTab === 'readers'

  const { data, isLoading } = useSWR(
    shouldFetch ? `/api/admin/translators/${translatorId}/readers` : null,
    (url) => fetch(url).then(r => r.json()),
    {
      revalidateOnFocus: false,
      dedupingInterval: 30000,
